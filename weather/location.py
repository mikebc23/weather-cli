...
class Coordinates:
    ...
    def _validate(self):
        ...
        if not (-90 <= self.lat <= 90):
            raise LocationError(f"Invalid latitude: {self.lat}. Must be between -90 and 90")
        
        if not (-180 <= self.lon <= 180):
            raise LocationError(f"Invalid longitude: {self.lon}. Must be between -180 and 180")
...
class LocationResolver:
    ...
    def _parse_coordinates(self, input_str: str) -> Coordinates:
        ...
        try:
            ...
            if len(parts) != 2:
                raise LocationError(f"Invalid coordinate format: {input_str}. Expected 'lat,lon' or 'lat lon'")
            ...
            raise LocationError(f"Invalid coordinate values: {input_str}. Must be numbers") from e
    ...
    def _parse_dms_coordinates(self, input_str: str) -> Coordinates:
        ...
        try:
            ...
            if not lat_match or not lon_match:
                raise LocationError(f"Could not find both latitude and longitude in: {input_str}")
    ...
    def _geocode_zipcode(self, zipcode: str) -> Coordinates:
        ...
        try:
            ...
            raise LocationError(f"Invalid geocoding response for ZIP code {zipcode}") from e
    ...
    def _geocode_place(self, place_name: str) -> Coordinates:
        ...
        try:
            ...
            raise LocationError(f"Invalid geocoding response for place {place_name}") from e
