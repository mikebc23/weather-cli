...
class WeatherConfig:
    ...
    def _validate_config(self) -> None:
        ...
        if self.config['units'] not in ['metric', 'imperial']:
            raise ConfigError(f"Invalid units: {self.config['units']}. Must be 'metric' or 'imperial'")
        
        # Validate format
        valid_formats = ['minimal', 'table', 'ascii', 'raw']
        if self.config['format'] not in valid_formats:
            raise ConfigError(f"Invalid format: {self.config['format']}. Must be one of {valid_formats}")
        
        # Validate source
        valid_sources = ['open-meteo', 'wttr', 'nws']
        if self.config['source'] not in valid_sources:
            raise ConfigError(f"Invalid source: {self.config['source']}. Must be one of {valid_sources}")
        
        # Validate numeric values
        if not isinstance(self.config['cache_duration'], int) or self.config['cache_duration'] < 0:
